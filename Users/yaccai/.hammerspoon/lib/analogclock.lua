...
seccolor      = {red=0.75,green=0.75,blue=0.75,alpha=0}     -- 秒针
mincolor      = {red=0.52,green=0.81,blue=1.00,alpha=0.75}   -- 分针
hourcolor     = {red=0.69,green=0.85,blue=0.90,alpha=0.75}   -- 时针

tofilledcolor = {red=0,   green=0,   blue=0,   alpha=0}   -- 未来小时分钟圈部分
secfillcolor  = {red=0.75,green=0.75,blue=0.75,alpha=0.25}   -- 秒针覆盖区域

if not aclockcenter then
    local mainScreen = hs.screen.mainScreen()
    local mainRes = mainScreen:fullFrame()
    aclockcenter = {x=130,y=150}
end

function showAnalogClock()
    if not bgcirle then
        imagerect = hs.geometry.rect(aclockcenter.x-100,aclockcenter.y-100,200,200)
        imagedisp = hs.drawing.image(imagerect,"./assets/watchbg.png")
...
function ClockManager:init()
    if not launch_analogclock then 
        launch_analogclock = true 
    end

    if launch_analogclock == true 
        then showAnalogClock() 
    end
end
